...
export default class Sprite {
  ...
  public constructor(gameitem: GameItem) {
    ...
    // how many gameloop frames we want to show this spritesheet
    this.animationFrameLimit = gameitem.getCurrentAnimationFrameLimit() || 8;
    // how much time is left untill the next image should be shown
    this.animationFrameProgress = this.animationFrameLimit;
  }

  /**
   * Method that determines which animation and frame we are on
   *
  ...
  public setAnimation(key: string) : void {
    ...
    // changes the current animation to the one being passed trough sets the animation frame back
    // to 0 sets the progress to the startpoint again aka the limit
  ...
  /**
   * This draws a gameitem which has a spritescheet
   *
   * @param ctx the game canvas
   * @param gameitem the game item we are currently drawing
   */
  public drawSprite(ctx: CanvasRenderingContext2D, gameitem: GameItem): void {
    ...
    // Separate the frame so the values can be used in drawimage
